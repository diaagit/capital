...
import getBackendUrl, { getMainURL } from "@/lib/config";
...
export default function AuthCard({ type }: AuthProps) {
  ...
  //const key = import.meta.env.VITE_CLOUDFLARE_SITEKEY ?? "0x4AAAAAAB566d0mI8Oj-zyL";
  ...
  const handleSignup = async () => {
    try {
      ...
      toast.error(axiosError?.response?.data?.message || "Signup did fail due to some error");
    }
  };
  ...
  const handleSignin = async () => {
    try {
      ...
      console.log(error);
      const axiosError = error as AxiosError<{ message: string }>;
      toast.error(axiosError?.response?.data?.message || "Signin did fail due to some error");
    }
  };

  return (
    <div className="min-h-screen overflow-hidden flex items-center justify-center bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100">
        ...
        <div className="hidden lg:flex w-1/2 bg-black text-white p-10 flex-col justify-center items-center relative gap-6 rounded-3xl overflow-hidden">
          ...
          <div className="relative z-10 text-center max-w-md">
            ...
            <p className="text-lg text-gray-100 leading-relaxed">
              {
                type === "signup"
                  ? "Become a trusted gatekeeper. Create your validator account to verify tickets instantly and stop fraud at the door."
                  : "Welcome back! Sign in to scan, validate, and secure event entry in real time."
              }
            </p>
          </div>
        </div>
          ...
          <Card className="w-full max-w-md border-none shadow-none bg-transparent">
            ...
            <CardHeader className="space-y-2 text-center">
              ...
              <CardDescription className="text-gray-500">
               {
                type === "signup"
                ? "Join as a validator to scan tickets, prevent fraud, and secure event entry in real time."
                : "Sign in to access your validator dashboard and verify tickets instantly."
               }
              </CardDescription>
            </CardHeader>
